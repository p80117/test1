...
{
    public static ushort cFile(String File_Name, String wData)//初版
    {
        /*
    ...
    public static ushort cFile(String File_Name, ushort [] wData)
    {
        StreamWriter sw = new StreamWriter(File_Name + ".txt", true, Encoding.Default);  //建立新檔案並將資料附加在後面
        //------Write Data To File-------------------------------------------------*
        foreach (ushort wr_Data in wData)
            sw.WriteLine(wr_Data);
        // Close File
        sw.Close();
        return 1;
    }
    public static ushort cFile(String File_Name, uint[] wData)
    {
        StreamWriter sw = new StreamWriter(File_Name + ".txt", true, Encoding.Default);  //建立新檔案並將資料附加在後面
        //------Write Data To File-------------------------------------------------*
        foreach (uint wr_Data in wData)
            sw.WriteLine(wr_Data);
        // Close File
        sw.Close();
        return 1;
...
{
    public static ushort switchCardName(String card_Name)
    {
        ushort CardName=9999;
        switch (card_Name)
        {
            case "PCI_9111DG":
                CardName = DASK.PCI_9111DG;
                break;
            case "PCI_9111HR":
                CardName = DASK.PCI_9111HR;
                break;
            case "PCI_9112":
                CardName = DASK.PCI_9112;                
                break;
            case "PCI_9113":
                CardName = DASK.PCI_9113;                
                break;
            case "PCI_9114DG" :
                CardName = DASK.PCI_9114DG;
                break;
            case "PCI_9114HG":
                CardName = DASK.PCI_9114HG;
                break;
            case "PCI_9524":
                CardName = DASK.PCI_9524;
                break;
        }
    ...
    public static ushort ADCGAIN(int n)
    {
        ushort a = 0;
        Double VP = 0;
    ...
    public static ushort ADCGAIN(string n)
    {
        ushort a = 0;
    ...
    public static ushort ADCGAIN(int n, out Double VP)
    {
        ushort a = 0;
        VP = 0;
    ...
    public static string[] cardGain(String card_Name)
    {
        string[] card_gain;
        //string[] card_gain;
        ...
        switch (card_Name)
        {
            case "PCI_9111DG":
            case "PCI_9111HR":
                card_gain = new string[] {"AD_B_10_V",
                    "AD_B_5_V",
                    "AD_B_2_5_V",
                    "AD_B_1_25_V",
                    "AD_B_0_625_V"
    ...
    */
    public static string[] sAICard(ushort cardID, ushort cardName)
    {
        ...
        log[3 * n] = "AI Config Fail=" + cardName+"err =";
    ...
    public static uint[] qc_AIContRead(ushort cardID, ushort AI_CH, int ADCRange, double ADC_SampRate, uint READCOUNT, int Card_Bit)
    {
        uint[] TEST = new uint[READCOUNT];        
          
                int err = DASK.AI_ContReadChannel(cardID, AI_CH, SKY_FUN_QC.ADCGAIN(ADCRange), TEST, READCOUNT, ADC_SampRate, 1); //CH
                ...
                for (int j = 0; j < READCOUNT; j++)
                {
                    switch (Card_Bit)
                    {
                        ...
                        case DASK.PCI_9114HG:
                        case DASK.PCI_9114DG:
                            TEST[j] = Convert.ToUInt32(TEST[j] & (0xFFFF)); //9111-HR 
                            break;
                        case DASK.PCI_9111DG:
                        case DASK.PCI_9112:
                            TEST[j] = Convert.ToUInt32((TEST[j] >> 4) & (0x0FFF)); //9112
                            break;
                        case DASK.PCI_9113:
                            TEST[j] = Convert.ToUInt32(TEST[j] & (0x0FFF)); //9113 
                            break;
                    }                
                }
                //----------------------------------------------------------------*
    ...
    public static ushort[] qc_AIContRead(ushort cardID, ushort AI_CH, ushort ADCRange, double ADC_SampRate, uint READCOUNT, ushort Card_Bit, int i)
    {
        ushort[] data_buffer = new ushort[READCOUNT];
        ushort[] AAA = new ushort [] {3, 3, 3};
        ...
        int err = DASK.AI_ContReadChannel((ushort)cardID, AI_CH, ADCRange, data_buffer, READCOUNT, ADC_SampRate, DASK.ASYNCH_OP); //CH
        //int err = DASK.AI_ContReadChannel(cardID, AI_CH, DASK.AD_B_10_V, TEST, READCOUNT, ADC_SampRate, DASK.SYNCH_OP); //CH
        if (err < 0) { throw new Exception("AI_AI_ContReadChannel Error = " + err); }  //由系統通知錯誤的視窗
        ...
        for (int j = 0; j < READCOUNT; j++)
        {
            switch (Card_Bit)
            {
                ...
                case DASK.PCI_9114DG:
                case DASK.PCI_9111HR:
                    data_buffer[j] = Convert.ToUInt16(data_buffer[j] & (0xFFFF)); //9111-HR                    
                    break;
                case DASK.PCI_9111DG:
                case DASK.PCI_9112:
                    data_buffer[j] = Convert.ToUInt16((data_buffer[j] >> 4) & (0x0FFF)); //9112
                    break;
                case DASK.PCI_9113:
                    data_buffer[j] = Convert.ToUInt16(data_buffer[j] & (0xFFF)); //9113 
                    break;
                case DASK.PCI_9524:
                    data_buffer[j] = Convert.ToUInt16(data_buffer[j] >> 8); //9524
                    break;
            }
        }
        //----------------------------------------------------------------*
    ...
    unsafe public static uint[] qc_AIContRead(ushort cardID, ushort AI_CH, ushort ADCRange, double ADC_SampRate, uint READCOUNT, ushort Card_Bit)
    {
        //uint[] data_buffer = new uint[READCOUNT];
        //int size = Marshal.SizeOf(data_buffer);

        uint[] data_buffer2 = new uint[READCOUNT];



        IntPtr data_buffer = Marshal.AllocHGlobal(4* (int)READCOUNT);
        //Marshal.StructureToPtr(data_buffer, buffer0, true);
        int err;      
        ...
        //for (int j = 0; j < READCOUNT; j++)   data_buffer[j] = 1;
        if(Card_Bit== DASK.PCI_9524)
        {
            err = DASK.AI_ContBufferSetup(cardID, data_buffer, READCOUNT, out BufID1);
            if (err < 0) throw new Exception("AI_ContBufferSetup Error = " + err);
        }
        
        err = DASK.AI_ContReadChannel((ushort)cardID, AI_CH, ADCRange,out BufID1, READCOUNT, ADC_SampRate, DASK.ASYNCH_OP); //CH
        //err = DASK.AI_ContReadChannel((ushort)cardID, AI_CH, 0, out BufID1, READCOUNT, ADC_SampRate, DASK.ASYNCH_OP); //CH
        if (err < 0) { throw new Exception("AI2_AI_ContReadChannel Error = " + err); }  //由系統通知錯誤的視窗
        
        do{err = DASK.AI_AsyncCheck((ushort)cardID, out stopped, out access_cnt);
            if (err < 0)    throw new Exception("AI_AsyncCheck Error = " + err);
        } while (stopped == 0);
        ...
        for (int j = 0; j < READCOUNT; j++)
        {
            ...
            {/*
                ...
                case DASK.PCI_9114DG:
                case DASK.PCI_9111HR:
                    data_buffer[j] = Convert.ToUInt32(data_buffer[j] & (0xFFFF)); //9111-HR                    
                    break;
                case DASK.PCI_9111DG:
                case DASK.PCI_9112:
                    data_buffer[j] = Convert.ToUInt32((data_buffer[j] >> 4) & (0x0FFF)); //9112
                    break;
                case DASK.PCI_9113:
                    data_buffer[j] = Convert.ToUInt32(data_buffer[j] & (0xFFF)); //9113 
                    break;*/
                case DASK.PCI_9524:
                    data_buffer2[j] = ((uint*)(void*)data_buffer)[j] >> 8; //9524
                    break;
            }
        }
        //----------------------------------------------------------------*
    ...
    public static Int16 qcSYS_NOISE(ushort cardID, ushort CardName, ushort AI_CH, ushort ADCRange, double ADC_SampRate, uint READCOUNT, String File_Name)
    {
        Int16 err = 0;
        ushort CH = AI_CH;
        uint[] TEST = new uint[READCOUNT];
        ...
        for (int i = 0; i < ADCRange; i++)  //測試不同Gain
        {
            ...
            for (CH = 0; CH <= AI_CH; CH++)
            {
                ...
                String wDate = "\nGroup \t#" + CH + "\tRange \t#" + ADCGAIN(i) + " \tADC_SampRate \t#" + ADC_SampRate;
    ...
    public static Int16 qcSYS_NOISE(ushort cardID, ushort CardName, ushort AI_CH, string [] ADCRange, double ADC_SampRate, uint READCOUNT, String File_Name)
    {
        Int16 err = 0;
        ushort CH = AI_CH;
        ...
        foreach(string s_ADCRANGE in ADCRange)  //測試不同Gain
        {
            ...
            for (CH = 0; CH <= AI_CH; CH++)
            {
                ...
                { 
                    ...
                    case DASK.PCI_9114HG:
                    uint[] data_buffer = new uint[READCOUNT];
                    data_buffer = SKY_FUN_QC.qc_AIContRead(cardID, CH, i, ADC_SampRate, READCOUNT, CardName); //抓取資料並轉換完畢              
                    //------Create Data Value-------------------------------------------------*	
                    SKY_FUN.cFile(SaveName, data_buffer);
                    //----------------------------------------------------------------*
                    break;
                    case DASK.PCI_9111DG:
                    case DASK.PCI_9111HR:
                    case DASK.PCI_9112:
                    ushort[] data_short_buf = new ushort[READCOUNT];
                    data_short_buf = SKY_FUN_QC.qc_AIContRead(cardID, CH, i, ADC_SampRate, READCOUNT, CardName,16); //抓取資料並轉換完畢              
                    //------Create Data Value-------------------------------------------------*	
                    SKY_FUN.cFile(SaveName, data_short_buf);
                    //----------------------------------------------------------------*
                    break;
    ...
    public static Int16 qcSYS_NOISE(ushort cardID, ushort CardName, ushort AI_CH, string[] ADCRange, double ADC_SampRate, uint READCOUNT, String File_Name, string TEST_Day, string TEST_SN)
    {
        /*History
         v0.0   2020/04/16  新增 TEST_SN, TEST_Day information in text
         */
        Int16 err = 0;
        ushort CH = AI_CH;

        string[] old_ADCRange = ADCRange;
        ...
        String SaveName;
            ...
            if (CardName == DASK.PCI_9524)
            {
                if (CH < 4)            
                    ADCRange = new string[] { "LC_B_0_2_V" };               
                else
                    ADCRange = old_ADCRange;
            }
            ...
            foreach (string s_ADCRANGE in ADCRange)  //測試不同Gain
            {
                ...
                if (CardName == DASK.PCI_9524)
                    err = DASK.AI_9524_Config(cardID, Convert.ToUInt16(CH/4), DASK.P9524_AI_XFER_DMA, DASK.P9524_AI_BufAutoReset, 0, 0); //PCI-9524 AI Config
                if (err < 0) { throw new Exception("AI_9524_Config Error = " + err); }  //由系統通知錯誤的視窗
                ...
                //------Create Save File-------------------------------------------------*
                SaveName = File_Name +"_"+ ADC_SampRate + "Hz" + "_G" + i + "_CH" + CH + ".txt";  //建立檔案名稱
                SKY_FUN.cFile(SaveName, "");
                String wDate = "\nSN \t" + TEST_SN + "\tTestDate \t" + TEST_Day + "\tGroup \t#" + CH + "\tRange \t#" + i + " \tADC_SampRate \t#" + ADC_SampRate;
                SKY_FUN.cFile(SaveName, wDate);
    ...
    public static Int16 qcAI_BW(int Fun_Num, ushort cardID, ushort AI_CH, ushort ADCRange, double ADC_SampRate, uint READCOUNT, String File_Name, int start_freq)
    {
        Int16 err;
        double volt_3db = 0;
        ushort[] m_data_buffer = new ushort[READCOUNT]; //資料存放的buffer
